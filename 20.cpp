...
signed main(){
	...
	FOR(t,0,n){
		...
		if(s == "PUT"){
			...
			cin>>a>>b;
			nth_element(arr.begin() + sz - a, arr.begin() + sz - (a+b-1)/b, arr.begin() + sz); 
			int sum = 0;
			FOR(i,sz - (a+b-1)/b,sz)
				sum += arr[i][0];
			cout<<sum<<endl;
			sort(arr.begin() + sz - a, arr.begin() + sz - (a+b-1)/b, [](vi &a,vi &b){return a[1] < b[1];});
			FOR(i,sz - a,sz - (a+b-1)/b){
				pre[i] = arr[i][0] + (i?pre[i-1]:0);
			}
			sz -= (a+b-1)/b;
		}
		else{
			cin>>a;
			cout<<pre[sz-1] - (sz-1-a >= 0?pre[sz-1-a]:0)<<endl;
		}
	}
	return 0;
}
