...
signed main(){
	...
	vvi cls(m,vi(3));
	FOR(i,0,m)
		cin>>cls[i][0]>>cls[i][1]>>cls[i][2];
	
	srand(time(NULL));
	vi ans(n);
	int sat = 0;
	do{
		FOR(i,0,n)ans[i] = (rand()%2) * 2 - 1;
		sat = 0;
		FOR(i,0,m)sat += (((ans[abs(cls[i][0])-1] ^ (cls[i][0])) >= 0) || ((ans[abs(cls[i][1])-1] ^ (cls[i][1])) >= 0) || ((ans[abs(cls[i][2])-1] ^ (cls[i][2])) >= 0));
	}
	while(8*sat < 7*m);
